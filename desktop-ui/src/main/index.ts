...
function setupIPC(): void {
  ...
  ipcMain.handle("get-runtime-path-config", () => getRuntimePathConfig());
  ipcMain.handle(
    "set-runtime-path-config",
    (
      _event,
      runtimeConfig: {
      ...
      setRuntimePathConfig(runtimeConfig);
      return true;
    },
  );
  ...
  ipcMain.handle("pick-file", async (_event, defaultPath?: string) => {
    const options: OpenDialogOptions = {
      properties: ["openFile"],
      defaultPath: defaultPath || undefined,
    };
