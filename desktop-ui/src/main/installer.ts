...
interface DesktopPathConfig {
  hermesRepoPath?: string;
  pythonPath?: string;
  hermesScriptPath?: string;
...
function readDesktopPathConfig(configFile: string): DesktopPathConfig {
  try {
    if (!existsSync(configFile)) return {};
    const parsed = JSON.parse(readFileSync(configFile, "utf-8"));
    if (!parsed || typeof parsed !== "object") return {};
    return {
      hermesRepoPath: normalizePath(parsed.hermesRepoPath),
      pythonPath: normalizePath(parsed.pythonPath),
      hermesScriptPath: normalizePath(parsed.hermesScriptPath),
...
function detectRepoFromCwd(): string | null {
  ...
  for (const candidate of candidates) {
    if (existsSync(join(candidate, "run_agent.py")) && existsSync(join(candidate, "hermes"))) {
      return candidate;
    }
  }
  return null;
...
function pythonCandidatesForRepo(repoPath: string): string[] {
  const binDir = IS_WINDOWS ? "Scripts" : "bin";
  const pythonName = IS_WINDOWS ? "python.exe" : "python";
  return [
    join(repoPath, ".venv", binDir, pythonName),
    join(repoPath, "venv", binDir, pythonName),
  ];
}
...
function resolveRuntimePaths(): RuntimePaths {
  ...
  const envPythonPath = normalizePath(process.env.HERMES_DESKTOP_PYTHON);
  const envScriptPath = normalizePath(process.env.HERMES_DESKTOP_SCRIPT);
  const envWorkspacePath = normalizePath(process.env.HERMES_DESKTOP_WORKSPACE);
  ...
  const scriptCandidates = [
    envScriptPath,
    diskConfig.hermesScriptPath,
    join(hermesRepo, "hermes"),
  ];
  const hermesScript =
    firstExistingPath(scriptCandidates) || scriptCandidates.find((v): v is string => Boolean(v)) || join(hermesRepo, "hermes");

  const pythonCandidates = [
    envPythonPath,
    diskConfig.pythonPath,
    ...pythonCandidatesForRepo(hermesRepo),
  ];
  const hermesPython =
    firstExistingPath(pythonCandidates) || pythonCandidates.find((v): v is string => Boolean(v)) || (IS_WINDOWS ? "python.exe" : "python");

  const workspaceCandidates = [envWorkspacePath, diskConfig.workspacePath, hermesRepo];
  const workspacePath =
...
const RESOLVED_RUNTIME_PATHS = resolveRuntimePaths();

export const HERMES_HOME = RESOLVED_RUNTIME_PATHS.hermesHome;
export const HERMES_REPO = RESOLVED_RUNTIME_PATHS.hermesRepo;
export const HERMES_VENV = RESOLVED_RUNTIME_PATHS.hermesVenv;
export const HERMES_PYTHON = RESOLVED_RUNTIME_PATHS.hermesPython;
export const HERMES_SCRIPT = RESOLVED_RUNTIME_PATHS.hermesScript;
export const HERMES_WORKSPACE = RESOLVED_RUNTIME_PATHS.workspacePath;
export const HERMES_ENV_FILE = join(HERMES_HOME, ".env");
export const HERMES_CONFIG_FILE = join(HERMES_HOME, "config.yaml");

export function getResolvedRuntimePaths(): RuntimePaths {
...
export async function runInstall(
  ...
  if (IS_WINDOWS) {
    emit("Windows detected. Using local Hermes runtime paths.\n");
    if (existsSync(HERMES_PYTHON) && existsSync(HERMES_SCRIPT)) {
      emit("Hermes runtime files found. Skipping shell installer.\n");
      return;
    }
    const details = [
      "Windows setup requires an existing Hermes checkout.",
      `Expected Python: ${HERMES_PYTHON}`,
      `Expected Hermes script: ${HERMES_SCRIPT}`,
      "Update these paths in Settings -> Local Runtime Paths, then restart the app.",
    ].join("\n");
