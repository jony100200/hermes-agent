...
export function getRuntimePathConfig(): RuntimePathConfig {
  const data = readDesktopConfig();
  return {
    hermesRepoPath: (data.hermesRepoPath as string) || "",
    pythonPath: (data.pythonPath as string) || "",
    hermesScriptPath: (data.hermesScriptPath as string) || "",
    workspacePath: (data.workspacePath as string) || "",
  };
}

export function setRuntimePathConfig(config: RuntimePathConfig): void {
  const data = readDesktopConfig();
  ...
  const repoPath = normalize(config.hermesRepoPath);
  const pythonPath = normalize(config.pythonPath);
  const scriptPath = normalize(config.hermesScriptPath);
  const workspacePath = normalize(config.workspacePath);

  data.hermesRepoPath = repoPath;
  data.pythonPath = pythonPath;
  data.hermesScriptPath = scriptPath;
