...
1. **Run tests**: use the platform wrapper (`scripts/run_tests.sh` on Linux/macOS/WSL, `pwsh -NoProfile -File .\scripts\run_tests.ps1` on Windows, recommended; same as CI) or `pytest tests/ -v` with the project venv activated
